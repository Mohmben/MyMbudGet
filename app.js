...
const catOf=id=>CATS.find(c=>c.id===id)||{emoji:'🧩',label:'Autres',id:'autres',color:'#64748b'};
...
function recKey(obj){ if (obj.key) return obj.key.trim(); return `${slug(obj.label)}|${(obj.category||'autres').toLowerCase()}|${(obj.freq||obj.frequency||'monthly').toLowerCase()}`; }
