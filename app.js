...
// ===== FAB & Intents: nouvelle fenêtre de saisie TX / Récurrence =====
// Crée deux boutons flottants : "+" pour transaction, "↻" pour récurrence
function setupPlusButtons() {
  ...
  // rétro-compat: ?new=1 -> on continue d'accepter, mais on émet ?new=tx
  urlTx.searchParams.set('new', 'tx');

  const urlRec = new URL(base);
  urlRec.searchParams.set('new', 'rec');

  // Ouvre une petite fenêtre centrée
...
// Interprète le paramètre ?new=...
// - "1" ou "tx" : aller au formulaire de transaction (onglet 'home' par défaut)
// - "rec" : aller au formulaire de récurrence (onglet 'manage')
function handleNewIntent() {
  ...
  if (val === '1' || val === 'tx') {
    ...
    setTab('home'); // si ton formulaire est ailleurs, change ici
    // Valeurs par défaut & focus
  ...
  if (val === 'rec') {
    ...
    setTab('manage'); // l’onglet qui contient recForm d’après ta structure
    // Valeurs par défaut & focus
    ...
    // Masquer/afficher zones fin suivant valeur par défaut
...
function init(){ 
  ...
  tabButtons.forEach(b=>on(b,'click',()=>setTab(b.dataset.tab)));
  buildCategoryDropdown(); 
  on(recEndType,'change',()=>{ if(recEndDate) recEndDate.classList.toggle('hidden',recEndType.value!=='until'); if(recCount) recCount.classList.toggle('hidden',recEndType.value!=='count'); });
  if(recEndDate) recEndDate.classList.add('hidden'); 
  if(recCount) recCount.classList.add('hidden'); 
  if(recCat) recCat.innerHTML=CATS.map(c=>`<option value="${c.id}">${c.emoji} ${c.label}</option>`).join('');
  if(budgetsList) budgetsList.innerHTML=CATS.map(c=>{ const v=budgets[c.id]??''; return `<div class="col"><label>${c.emoji} ${c.label}</label><input class="input" id="bud_${c.id}" placeholder="Budget mensuel (ex: 200)" value="${v}"></div>`}).join('');
  on(window,'resize',()=>render()); 
  on(document.getElementById('prev'),'click',()=>changeMonth(-1)); 
  on(document.getElementById('next'),'click',()=>changeMonth(1)); 
  on(form,'submit',addTx); 
  on(recForm,'submit',addRecurrence); 
  on(saveBudgetsBtn,'click',saveBudgets);

  // NEW: deux boutons flottants (TX & Recurrence) + gestion des intents
  setupPlusButtons();
  handleNewIntent();
